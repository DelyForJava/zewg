...
//	http://www.opsive.com
//
//	description:	Timer is a script extension for delaying (scheduling) methods
...
public class Timer : MonoBehaviour
{
    ...
    /// <summary>
    /// returns false if this script was not added via the 'Timer.In'
    ...
    /// <summary>
    /// in Awake a check is made to see if the Timer component
    ...
    private void Update()
    {
        ...
        m_EventBatch = 0;
        while ((Timer.m_Active.Count > 0) && m_EventBatch < MaxEventsPerFrame)
        {
            ...
            if (m_EventIterator < 0)
            {
                ...
                m_EventIterator = Timer.m_Active.Count - 1;
                break;
            }

            // prevent index out of bounds
            if (m_EventIterator > Timer.m_Active.Count - 1)
                m_EventIterator = Timer.m_Active.Count - 1;

            // execute all due events
            if (Time.time >= Timer.m_Active[m_EventIterator].DueTime || // time is up
                Timer.m_Active[m_EventIterator].Id == 0)                    // event has been canceled ('Execute' will kill it)
                Timer.m_Active[m_EventIterator].Execute();
            else
            {
                // handle pausing
                if (Timer.m_Active[m_EventIterator].Paused)
                    Timer.m_Active[m_EventIterator].DueTime += Time.deltaTime;
                else
                    // log lifetime
                    Timer.m_Active[m_EventIterator].LifeTime += Time.deltaTime;
            }
    ...
    private static void Schedule(float time, Callback func, ArgCallback argFunc, object args, Handle timerHandle, int iterations, float interval)
    {
        ...
        {
            UnityEngine.Debug.LogError("Error: (Timer) Aborted event because function is null.");
            return;
        }
        ...
        if (m_MainObject == null)
        {
            ...
            if (!m_AppQuitting)
            {
                ...
                m_MainObject.AddComponent<Timer>();
        ...
        Timer.m_EventCount++;
        m_NewEvent.Id = Timer.m_EventCount;
    ...
    public static void CancelAll()
    {
        ...
        for (int t = Timer.m_Active.Count - 1; t > -1; t--)
        {
            Timer.m_Active[t].Id = 0;
        }
    ...
    public static void CancelAll(string methodName)
    {
        ...
        for (int t = Timer.m_Active.Count - 1; t > -1; t--)
        {
            if (Timer.m_Active[t].MethodName == methodName)
                Timer.m_Active[t].Id = 0;
        }
...
#endif
    {
        ...
        for (int t = Timer.m_Active.Count - 1; t > -1; t--)
        {
            if (Timer.m_Active[t].CancelOnLoad)
                Timer.m_Active[t].Id = 0;
        }
    ...
    /// <summary>
    /// provides the custom Timer editor class with debug info
    /// </summary>
    ...
    /// <summary>
    /// provides the custom Timer editor class with the name of a
    ...
    /// <summary>
    /// provides the custom Timer editor class with the id of a
    ...
    //					an event post initiation, schedule it with a 'Timer.Handle'
    //
    /////////////////////////////////////////////////////////////////////////////////
    private class Event
    {
        ...
        /// <summary>
        /// performs internal recycling of the Timer
        /// </summary>
        ...
        private void Destroy()
        {
            ...
            Timer.m_Active.Remove(this);
            Timer.m_Pool.Remove(this);
    ...
    public class Handle
    {
        ...
        private Timer.Event m_Event = null; // timer we're pointing at
        ...
        public int Id
        {
            ...
            set
            {
                ...
                for (int t = Timer.m_Active.Count - 1; t > -1; t--)
                {
                    if (Timer.m_Active[t].Id == m_Id)
                    {
                        m_Event = Timer.m_Active[t];
                        break;
                    }
                }
                if (m_Event == null)
